...
Commontator.configure do |config|
  ...
  p 'lambda - ' + lambda { |controller| controller.current_user }.to_s
  ...
  config.user_name_proc = lambda { |user| user.uid }
  ...
  config.commontable_name_proc = lambda { |thread|
    "#{thread.commontable.class.name} ##{thread.table_commontable.id}" }
  ...
  config.commontable_url_proc = lambda { |thread, app_routes|
   "cnn.com" }
  #app_routes.polymorphic_url(thread.table_commontable) }
end
